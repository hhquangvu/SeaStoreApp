(function() {
	function create(window) {
		...
		if (!window || !window.document) {
			...
			// trick jQuery into thinking CORS is supported (should be in
			// node-XMLHttpRequest)
		...
		var location = window.location, navigator = window.navigator, XMLHttpRequest = window.XMLHttpRequest;
		 ...
		 * 
		 * Copyright 2012 jQuery Foundation and other contributors Released
		...
		(function(window, undefined) {
			...
			jQuery
					.extend({
						...
						// data: string of html
						// context (optional): If specified, the fragment will
						// be created in this context, defaults to document
						// scripts (optional): If true, will include scripts
						...
						parseHTML : function(data, context, scripts) {
							...
							parsed = jQuery.buildFragment([ data ], context,
									scripts ? null : []);
							return jQuery.merge([],
									(parsed.cacheable ? jQuery
											.clone(parsed.fragment)
						...
						// collection
						// The value/s can optionally be executed if it's a
						// function
						access : function(elems, fn, key, value, chainable,
							...
							if (key && typeof key === "object") {
								...
								if (bulk) {
									// Bulk operations only iterate when
									// executing function values
									if (exec) {
										exec = fn;
			...
			jQuery.ready.promise = function(obj) {
				if (!readyList) {
					...
					// we once tried to use readyState "interactive" here, but
					// it caused issues like the one
					...
					if (document.readyState === "complete") {
						...
						// Ensure firing before onload, maybe late but safe also
						// for iframes
			...
			// Convert String-formatted options into Object-formatted ones and
			// store in cache
			 ...
			 * 
			 * options: an optional list of space-separated options that will
			 * change how the callback list behaves or a more traditional option
			 * object
			 * 
			 * By default a callback list will act like an event callback list
			 ...
			 * latest "memorized" values (like a Deferred)
			 * 
			...
			jQuery.Callbacks = function(options) {
				...
				// Convert options from String-formatted to Object-formatted if
				// needed
				...
				fire = function(data) {
					...
					for (; list && firingIndex < firingLength; firingIndex++) {
						if (list[firingIndex].apply(data[0], data[1]) === false
								&& options.stopOnFalse) {
							memory = false; // To prevent further calls using
											// add
							break;
						}
					}
			...
			jQuery.support = (function() {
				...
				// Support tests won't run in some limited or non-browser
				// environments
				...
				// Make sure that the options inside disabled selects aren't
				// marked as disabled
				// (WebKit marks them as disabled)
				select.disabled = true;
				support.optDisabled = !opt.disabled;

				// Test to see if it's possible to delete an expando from an
				// element
				...
				// #11217 - WebKit loses check when the name is after the
				// checked attribute
				...
				jQuery(function() {
					...
					// Check if table cells still have offsetWidth/Height when
					// they are set
			...
			jQuery
					.extend({
						...
						expando : "jQuery"
								+ (jQuery.fn.jquery + Math.random()).replace(
										/\D/g, ""),
						...
						data : function(elem, name, data, pvt /*
							...
							// the code to shortcut on the same path as a DOM
							// node with no cache
							...
							if (!id) {
								...
								if (isNode) {
									elem[internalKey] = id = jQuery.deletedIds
											.pop()
											|| jQuery.guid++;
								} else {
									id = internalKey;
							...
							if (!cache[id]) {
								...
								// Avoids exposing jQuery metadata on plain JS
							...
							// An object can be passed to jQuery.data instead of
							...
							// jQuery data() is stored in a separate object
							...
							// Check for both converted-to-camel and
							// non-converted data property names
						...
						removeData : function(elem, name, pvt /*
							...
							// If there is already no cache entry for this
							// object, there is no
							...
							if (name) {
								...
								if (thisCache) {
									...
									// want to continue
									// and let the cache object itself get
									// destroyed
							...
							if (isNode) {
								...
								// Use delete when supported for expandos or
								// `cache` is not a window per isWindow (#10080)
			...
			function dataAttr(elem, key, data) {
				...
				if (data === undefined && elem.nodeType === 1) {
					...
					var name = "data-"
							+ key.replace(rmultiDash, "-$1").toLowerCase();
					...
					.extend({
						queue : function(elem, type, data) {
							...
							if (elem) {
								...
								// Speed up dequeue by getting out quickly if
						...
						dequeue : function(elem, type) {
							...
							var queue = jQuery.queue(elem, type), startLength = queue.length, fn = queue
									.shift(), hooks = jQuery._queueHooks(elem,
									type), next = function() {
						...
						// not intended for public consumption - generates a
						// queueHooks object, or returns the current one
			...
			jQuery.fn
					.extend({
						queue : function(type, data) {
							...
							return data === undefined ? this : this
									.each(function() {
										var queue = jQuery.queue(this, type,
												data);
			...
			jQuery.fn
					.extend({
						attr : function(name, value) {
							return jQuery.access(this, jQuery.attr, name,
							...
							if ((value && typeof value === "string")
								...
								for (i = 0, l = this.length; i < l; i++) {
									...
									if (elem.nodeType === 1 && elem.className) {
										...
										// loop over each item in the removal
										// list
										for (c = 0, cl = removes.length; c < cl; c++) {
											// Remove until there is nothing to
											// remove,
											while (className.indexOf(" "
													+ removes[c] + " ") >= 0) {
						...
						val : function(value) {
							...
							return this
									.each(function(i) {
										...
										// If set returns undefined, fall back
										// to normal setting
										if (!hooks
												|| !("set" in hooks)
												|| hooks
														.set(this, val, "value") === undefined) {
											this.value = val;
										}
									});
						}
			...
			jQuery
					.extend({
						valHooks : {
							option : {
								get : function(elem) {
									// attributes.value is undefined in
									// Blackberry 4.7 but
									// uses .value. See #6932
							...
							select : {
								get : function(elem) {
									...
									for (; i < max; i++) {
										...
										// oldIE doesn't update selected after
										// form reset (#2551)
										if ((option.selected || i === index)
											...
											// Get the specific value for the
											// option
						...
						// Unused in 1.8, left in so attrFn-stabbers won't die;
						// remove in 1.9
						attrFn : {},
							...
							if (value && elem.nodeType === 1) {
								...
								for (; i < attrNames.length; i++) {
									...
									if (name) {
										...
										// approach (setting first, then
										// removal)
										// Do not do this for boolean attributes
										// (see #10870)
										if (!isBool) {
						...
						attrHooks : {
							type : {
								set : function(elem, value) {
									...
									if (rtype.test(elem.nodeName)
										...
										// after the value resets the value in
										// IE6-9
							...
							// Use the nodeHook for button elements in IE6/7
							// (#1954)
							value : {
							...
							tabIndex : {
								get : function(elem) {
									...
									return attributeNode
											&& attributeNode.specified ? parseInt(
			...
			boolHook = {
				get : function(elem, name) {
					...
					// Fall back to attribute presence where some booleans are
					// not supported
			...
			if (!jQuery.support.hrefNormalized) {
				jQuery.each([ "href", "src", "width", "height" ], function(i,
						name) {
					jQuery.attrHooks[name] = jQuery.extend(
							jQuery.attrHooks[name], {
								get : function(elem) {
			 ...
			 * interface. Props to Dean Edwards' addEvent library for many of
			 * the ideas.
			 */
			jQuery.event = {
				...
				add : function(elem, types, handler, data, selector) {
					...
					// (allow plain objects tho)
					if (elem.nodeType === 3 || elem.nodeType === 8 || !types
							|| !handler || !(elemData = jQuery._data(elem))) {
						return;
					}

					// Caller can pass in an object of custom data in lieu of
					// the handler
					if (handler.handler) {
						handleObjIn = handler;
					...
					// Init the element's event structure and main handler, if
					// this is the first
				...
				remove : function(elem, types, handler, selector, mappedTypes) {
					...
					// Once for each type.namespace in types; type may be
					// omitted
					types = jQuery.trim(hoverHack(types || "")).split(" ");
					for (t = 0; t < types.length; t++) {
						tns = rtypenamespace.exec(types[t]) || [];
				...
				trigger : function(event, data, elem, onlyHandlers) {
					...
					if ((!elem || jQuery.event.customEvent[type])
						...
						// No jQuery handlers for this event type, and it can't
						// have inline handlers
						return;
					}

					// Caller can pass in an Event, Object, or just an event
					// type string
					...
					// Clone any incoming data and prepend the event, creating
					// the handler arg list
					...
					// Determine event propagation path in advance, per W3C
					// events spec (#9951)
					...
					if (!onlyHandlers && !special.noBubble
						...
						if (old === (elem.ownerDocument || document)) {
							eventPath.push([
									old.defaultView || old.parentWindow
											|| window, bubbleType ]);
						}
					}
					...
					if (!onlyHandlers && !event.isDefaultPrevented()) {
						...
						if ((!special._default || special._default.apply(
							...
							// Call a native DOM method on the target with the
							// same name name as the event.
							// Can't use an .isFunction() check here because
							// IE6/7 fails that test.
							// Don't do default actions on window, that's where
							...
							if (ontype
								...
								// since we already bubbled it above
				...
				dispatch : function(event) {
					...
					if (delegateCount
						...
						for (cur = event.target; cur != this; cur = cur.parentNode
							...
							// Don't process clicks (ONLY) on disabled elements
					...
					for (i = 0; i < handlerQueue.length
						...
						for (j = 0; j < matched.matches.length
							...
							// Triggered event must either 1) be non-exclusive
							// and have no namespace, or
				...
				// *** attrChange attrName relatedNode srcElement are not
				// normalized, non-W3C, deprecated, will be removed in 1.8 ***
				...
				fix : function(event) {
					...
					// Fix target property, if necessary (#1925, IE 6/7/8 &
					// Safari2)
					if (!event.target) {
					...
					return fixHook.filter ? fixHook
							.filter(event, originalEvent) : event;
				},
				...
				simulate : function(type, elem, event, bubble) {
					...
					// Fake originalEvent to avoid donor's stopPropagation, but
					// if the
			...
			jQuery.removeEvent = document.removeEventListener ? function(elem,
				...
				if (elem.detachEvent) {
					...
					// #8545, #7054, preventing memory leaks for custom events
					// in IE6-8
					// detachEvent needed property on element, by name of that
					// event, to properly expose it to GC
							...
							function(orig, fix) {
								jQuery.event.special[orig] = {
									...
									handle : function(event) {
										...
										// For mousenter/leave call the handler
			...
			if (!jQuery.support.submitBubbles) {
				...
				jQuery.event.special.submit = {
					...
					teardown : function() {
						...
						// Remove delegated handlers; cleanData eventually reaps
						// submit handlers attached above
			...
			if (!jQuery.support.changeBubbles) {
				...
				jQuery.event.special.change = {
					...
					handle : function(event) {
						...
						// Swallow native change events from checkbox/radio, we
						// already triggered them above
						if (this !== elem
								|| event.isSimulated
								|| event.isTrigger
			...
			jQuery.fn
					.extend({
						...
						live : function(types, data, fn) {
							jQuery(this.context).on(types, this.selector, data,
									fn);
							return this;
						},
						die : function(types, fn) {
							jQuery(this.context).off(types,
									this.selector || "**", fn);
							return this;
						},
						...
						toggle : function(fn) {
							...
							var args = arguments, guid = fn.guid
									|| jQuery.guid++, i = 0, toggler = function(
									event) {
								// Figure out which function to execute
								var lastToggle = (jQuery._data(this,
			...
			/*
			 * ! Sizzle CSS Selector Engine Copyright 2012 jQuery Foundation and
			...
			(function(window, undefined) {
				...
				createCache = function() {
					...
					return markFunction(function(key, value) {
						...
						// Retrieve with (key + " ") to avoid collision with
						// native Object.prototype properties (see Issue #157)
				...
				assertAttributes = assert(function(div) {
					...
					// IE8 returns a string for some attributes even when not
					// present
				...
				assertUsableClassName = assert(function(div) {
					...
					// Safari 3.2 caches class attributes and doesn't catch
					// changes
				...
				// Check if getElementsByName privileges form controls or
						...
						if ((match = rquickExpr.exec(selector))) {
							...
							if ((m = match[1])) {
								if (nodeType === 9) {
									...
									// #6963
									if (elem && elem.parentNode) {
										// Handle the case where IE, Opera, and
										// Webkit return items
										// by name instead of ID
										if (elem.id === m) {
											results.push(elem);
				...
				getText = Sizzle.getText = function(elem) {
					...
					if (nodeType) {
						if (nodeType === 1 || nodeType === 9 || nodeType === 11) {
							...
							// innerText usage removed for consistency of new
							// lines (see #11153)
				...
				Expr = Sizzle.selectors = {
					...
					preFilter : {
						"ATTR" : function(match) {
							...
							// Move the given value to match[3] whether quoted
							// or unquoted
						...
						"CHILD" : function(match) {
							 ...
							 * (only|nth|...) 2 argument
							 * (even|odd|\d*|\d*n([+-]\d+)?|...) 3 xn-component
							 * of xn+y argument ([+-]?\d*n|) 4 sign of
							 * xn-component 5 x of xn-component 6 sign of
						...
						"CHILD" : function(type, argument, first, last) {
							...
							if (type === "nth") {
								return function(elem) {
									...
									// Incorporate the offset (or cast to NaN),
									// then check against cycle size
									diff -= last;
									return diff === first
											|| (diff % first === 0 && diff
													/ first >= 0);
					...
					pseudos : {
						...
						"empty" : function(elem) {
							...
							// :empty is only affected by element nodes and
							// content nodes(including text(3), cdata(4)),
						...
						"text" : function(elem) {
							var type, attr;
							// IE6 and 7 will map elem.type to 'text' for new
							// HTML5 types (search, etc)
						...
						"eq" : createPositionalPseudo(function(matchIndexes,
								length, argument) {
				...
				sortOrder = docElem.compareDocumentPosition ? function(a, b) {
					...
					return (!a.compareDocumentPosition
							|| !b.compareDocumentPosition ? a.compareDocumentPosition
						...
						: function(a, b) {
							...
							// If the nodes are siblings (or identical) we can
							// do a quick check
							if (aup === bup) {
							...
							// Otherwise they're somewhere else in the tree so
							// we need
							// to build up a full list of the parentNodes for
							// comparison
							while (cur) {
								ap.unshift(cur);
								cur = cur.parentNode;
							...
							// Start walking down the tree looking for a
							// discrepancy
							...
							// We ended someplace up the tree so do a sibling
							// check
					...
					function(elem, context, xml) {
						while ((elem = elem[dir])) {
							...
							function(elem, context, xml) {
								...
								if (!xml) {
									var cache, dirkey = dirruns + " "
											+ doneName + " ", cachedkey = dirkey
											+ cachedruns;
									while ((elem = elem[dir])) {
										if (checkNonElements
												|| elem.nodeType === 1) {
					...
					return markFunction(function(seed, results, context, xml) {
						...
						if (seed) {
							if (postFinder || preFilter) {
								if (postFinder) {
									// Get the final matcherOut by condensing
									// this intermediate into postFinder
									// contexts
									temp = [];
									i = matcherOut.length;
									while (i--) {
								...
								// Move matched elements from seed to results to
							...
							// Add elements to results, through postFinder if
							// defined
						} else {
				...
				function matcherFromTokens(tokens) {
					...
					// The foundational matcher ensures that elements are
					// reachable from top-level context(s)
				...
				compile = Sizzle.compile = function(selector, group /*
					...
					if (!cached) {
						// Generate a function of recursive functions that can
						// be used to check each element
						if (!group) {
				...
				function select(selector, context, results, seed, xml) {
						...
						if (match.length === 1) {
							...
							if (tokens.length > 2
								...
								context = Expr.find["ID"](token.matches[0]
										.replace(rbackslash, ""), context, xml)[0];
								if (!context) {
									return results;
								}

								selector = selector
										.slice(tokens.shift().length);
							}
							...
							for (i = matchExpr["POS"].test(selector) ? -1
								...
								if ((find = Expr.find[type])) {
									...
									if ((seed = find(token.matches[0].replace(
										...
										// If seed is empty or no tokens remain,
										// we can return early
										tokens.splice(i, 1);
					...
					// Provide `match` to avoid retokenization if we modified
					// the selector above
				...
				if (document.querySelectorAll) {
					(function() {
						...
						// qSa(:focus) reports false when true (Chrome 21), no
						// need to also add to buggyMatches since matches checks
						// buggyQSA
						// A support test would require too much code (would
						...
						assert(function(div) {
							...
							// IE8 - Some boolean attributes are not treated
							// correctly
							...
							// option elements
							// http://www.w3.org/TR/2011/REC-css3-selectors-20110929/#checked
							// IE8 throws error here (do not put tests after
							// this one)
						...
						// rbuggyQSA always contains :focus, so no need for a
						// length check
						...
						select = function(selector, context, results, seed, xml) {
							...
							return oldSelect(selector, context, results, seed,
									xml);
						};
						...
						if (matches) {
							...
							// rbuggyMatches always contains :active and :focus,
							// so no need for a length check
							...
							Sizzle.matchesSelector = function(elem, expr) {
								...
								// rbuggyMatches always contains :active, so no
								// need for an existence check
								if (!isXML(elem) && !rbuggyMatches.test(expr)
										&& !rbuggyQSA.test(expr)) {
									try {
										var ret = matches.call(elem, expr);

										// IE 9's matchesSelector returns false
										// on disconnected nodes
										if (ret
												|| disconnectedMatch
												||
												// As well, disconnected nodes
												// are said to be in a document
												// fragment in IE 9
												elem.document
					...
					.extend({
						...
						not : function(selector) {
							return this.pushStack(
									winnow(this, selector, false), "not",
									selector);
						},
						...
						is : function(selector) {
							return !!selector
									...
									rneedsContext.test(selector) ? jQuery(
											selector, this.context).index(
											this[0]) >= 0 : jQuery.filter(
					...
					.each(
							{
								...
								jQuery.fn[name] = function(until, selector) {
									...
									return this.pushStack(ret, name, core_slice
											.call(arguments).join(","));
								};
							});
			...
			// tags,
			// unless wrapped in a div with non-breaking characters in front of
			// it.
			...
			jQuery.fn
					.extend({
						...
						after : function() {
							...
							if (arguments.length) {
								var set = jQuery.clean(arguments);
								return this.pushStack(jQuery.merge(this, set),
										"after", this.selector);
							}
						},
						...
						replaceWith : function(value) {
							if (!isDisconnected(this[0])) {
								// Make sure that the elements are removed from
								// the DOM before they are inserted
								// this can help fix replacing a parent with
								// child elements
								if (jQuery.isFunction(value)) {
									return this.each(function(i) {
						...
						domManip : function(args, table, callback) {
							...
							if (this[0]) {
								...
								if (first) {
									table = table
											&& jQuery.nodeName(first, "tr");

									// Use the original fragment for the last
									// item instead of the first because it can
									// end up
									// being emptied incorrectly in certain
									// situations (#8070).
									// Fragments from the fragment cache must
									// always be cloned and never used in place.
			...
			function cloneFixAttributes(src, dest) {
				...
				if (nodeName === "object") {
					// IE6-10 improperly clones children of object elements
					// using classid.
					// IE10 throws NoModificationAllowedError if parent is null,
					// #12132.
					if (dest.parentNode) {
						dest.outerHTML = src.outerHTML;
					}

					// This path appears unavoidable for IE9. When cloning an
					// object
					// element in IE9, the outerHTML strategy above is not
					// sufficient.
					...
					// IE6-8 fails to persist the checked state of a cloned
					// checkbox
					// or radio button. Worse, IE6-7 fail to give the cloned
					// element
					// a checked appearance if the defaultChecked value isn't
					// also set

					dest.defaultChecked = dest.checked = src.checked;

					// IE6-7 get confused and end up setting the value of a
					// cloned
					...
					// IE6-8 fails to set the defaultValue to the correct value
					// when
			...
			jQuery.buildFragment = function(args, context, scripts) {
				...
				// Updated to fix #12266 where accessing context[0] could throw
				...
				// that were created from unknown elems #10501
				if (args.length === 1
						&& typeof first === "string"
						&& first.length < 512
						&& context === document
						...
						&& (jQuery.support.html5Clone || !rnoshimcache
								.test(first))) {
					...
					.extend({
						clone : function(elem, dataAndEvents, deepDataAndEvents) {
							...
							if (jQuery.support.html5Clone
								...
								// IE<=8 does not properly clone detached,
								// unknown element nodes
							} else {
							...
							if ((!jQuery.support.noCloneEvent || !jQuery.support.noCloneChecked)
								...
								// using cloneNode.
								// Calling detachEvent on the clone will also
								// remove the events
								// from the original. In order to get around
								// this, we use some
								// proprietary methods to clear the events.
								// Thanks to MooTools
								...
								// length property
								// with an element if you are cloning the body
								// and one of the
								// elements on the page has a name or id of
								// "length"
								for (i = 0; srcElements[i]; ++i) {
									// Ensure that the destination node is not
									// null; Fixes #9587
									if (destElements[i]) {
						...
						clean : function(elems, context, fragment, scripts) {
							...
							// Use the already-created safe fragment if context
							// permits
							for (i = 0; (elem = elems[i]) != null; i++) {
								...
								if (typeof elem === "string") {
									if (!rhtml.test(elem)) {
										...
										// IE completely kills leading
										// whitespace when innerHTML is used
										...
										// Take out of fragment container (we
										// need a fresh div each time)
							...
							if (fragment) {
								...
								handleScript = function(elem) {
									...
									if (!elem.type
										...
										// Detach the script and store it in the
										// scripts array (if provided) or the
										// fragment
										// Return truthy to indicate that it has
										// been handled
								...
								for (i = 0; (elem = ret[i]) != null; i++) {
									...
									if (!(jQuery.nodeName(elem, "script") && handleScript(elem))) {
										...
										fragment.appendChild(elem);
										if (typeof elem.getElementsByTagName !== "undefined") {
											// handleScript alters the DOM, so
											// use jQuery.merge to ensure
											// snapshot iteration
											jsTags = jQuery
													.grep(
															jQuery
											...
											// Splice the scripts into ret after
											// their former ancestor and advance
											// our index beyond them
							...
							for (; (elem = elems[i]) != null; i++) {
								...
								if (acceptData || jQuery.acceptData(elem)) {
									...
									if (data) {
										...
										// Remove cache only if it was not
										// already removed by
										// jQuery.event.remove
										if (cache[id]) {
											...
											// IE does not allow us to delete
											// expando properties from nodes,
											// nor does it have a
											// removeAttribute function on
											// Document nodes;
					...
					.extend({
						...
						style : function(elem, name, value, extra) {
							...
							if (value !== undefined) {
								...
								// Make sure that NaN and null values aren't
								// set. See: #7116
								...
								// (except for certain CSS properties)
								if (type === "number"
								...
								if (!hooks
									...
									// errors when 'invalid' values are provided
									// Fixes bug #5509
									try {
										style[name] = value;
									} catch (e) {
						...
						css : function(elem, name, numeric, extra) {
							...
							// Return, converting to number if forced or a
							// qualifier was provided and val looks numeric
						...
						// A method for quickly swapping in/out CSS properties
			...
			if (window.getComputedStyle) {
				curCSS = function(elem, name) {
					...
					if (computed) {
						...
						// larger set of values, but width seems to be reliably
						// pixels
			...
			function augmentWidthOrHeight(elem, name, extra, isBorderBox) {
				...
				for (; i < 4; i += 2) {
					...
					if (isBorderBox) {
						...
						// content
						if (extra === "content") {
							val -= parseFloat(curCSS(elem, "padding"
									+ cssExpand[i])) || 0;
						}
			...
			function getWidthOrHeight(elem, name, extra) {
				...
				// Start with offset property, which is equivalent to the
				// border-box value
				var val = name === "width" ? elem.offsetWidth
						: elem.offsetHeight, valueIsBorderBox = true, isBorderBox = jQuery.support.boxSizing
						&& jQuery.css(elem, "boxSizing") === "border-box";

				// some non-html elements return undefined for offsetWidth, so
				...
				// use the active box-sizing model to add/subtract irrelevant
				// styles
			...
			function css_defaultDisplay(nodeName) {
				...
				if (display === "none" || display === "") {
					...
					elem = iframeDoc.body.appendChild(iframeDoc
								...
								jQuery.cssHooks[name] = {
									get : function(elem, computed, extra) {
										if (computed) {
											...
											// dimension info if we invisibly
											// show them
											// however, it must have a current
											// display style that would benefit
											// from this
				...
				jQuery.cssHooks.opacity = {
					...
					set : function(elem, value) {
						var style = elem.style, currentStyle = elem.currentStyle, opacity = jQuery
								.isNumeric(value) ? "alpha(opacity=" + value
								* 100 + ")" : "", filter = currentStyle
						...
						if (value >= 1
							...
							// if there there is no filter style applied in a
							// css rule, we are done
						...
						style.filter = ralpha.test(filter) ? filter.replace(
								ralpha, opacity) : filter + " " + opacity;
					}
				};
			}

			// These hooks cannot be added until DOM ready because the support
			// test
			...
			jQuery(function() {
				...
				// rather than make the css module depend on the offset module,
				...
				if (jQuery.isArray(obj)) {
					...
					jQuery.each(obj, function(i, v) {
						if (traditional || rbracket.test(prefix)) {
							...
							// If array item is non-scalar (array or object),
							// encode its
							// numeric index to resolve deserialization
							// ambiguity issues.
							// Note that rack (as of 1.0.0) can't currently
							// deserialize
							// nested arrays properly, and attempting to do so
							// may cause
							// a server error. Possible fixes are to modify
							// rack's
							// deserialization algorithm or to provide an option
							// or flag
			 ...
			 * ajax/jsonp.js for an example) 2) These are called: - BEFORE
			 * asking for a transport - AFTER param serialization (s.data is a
			 * string if s.processData is true) 3) key is the dataType 4) the
			 * catchall symbol "*" can be used 5) execution will start with
			 ...
			 * "*" can be used 3) selection will start with transport dataType
			 * and THEN go to "*" if needed
			 */
			transports = {},

			// Avoid comment-prolog char sequence (#10098); must appease lint
			// and evade compression
			allTypes = [ "*/" ] + [ "*" ];
			...
			jQuery
					.extend({
						...
						ajax : function(url, options) {
							...
							// It is defined here because jslint complains if it
							// is declared
							...
							// promotion)
							// Add protocol if not provided (#5866: IE7 issue
							...
							// Apply prefilters
							inspectPrefiltersOrTransports(prefilters, s,
									options, jqXHR);
							...
							if (!s.hasContent) {
								...
								// Get ifModifiedKey before adding the
								...
								if (s.cache === false) {
									...
									// if nothing was replaced, add timestamp to
									// the end
									s.url = ret
											+ ((ret === s.url) ? (rquery
													.test(s.url) ? "&" : "?")
							...
							if (s.beforeSend
									&& (s.beforeSend.call(callbackContext,
			...
			/*
			 * Handles responses to an ajax request: - sets all responseXXX
			 * fields accordingly - finds the right dataType (mediates between
			 * content-type and expected dataType) - returns the corresponding
			 * response
			 */
			...
			function ajaxConvert(s, response) {
				...
				// Convert to each sequential dataType, tolerating list
				// modification
					...
					if (current !== "*") {
						...
						if (prev !== "*" && prev !== current) {
							...
							if (conv !== true) {
								...
								// Unless errors are allowed to bubble, catch
								// and return them
								if (conv && s["throws"]) {
								...
								if (s.dataTypes[0] === "jsonp" || replaceInUrl
									...
									jqXHR
											.always(function() {
												...
												// Save back as free
												if (s[callbackName]) {
													// make sure that re-using
													// the options doesn't screw
													// things around
								...
								if (s.crossDomain) {
									...
									return {
										...
										send : function(_, callback) {
											...
											// appendChild to circumvent an IE6
											// bug.
											// This arises when a base node is
											// used (#2709 and #4378).
			 ...
			 * (can't request local files), so we use the ActiveXObject when it
			 * is available Additionally XMLHttpRequest can be disabled in
			 * IE7/IE8 so we need a fallback.
			 */
			function() {
							...
							if (!s.crossDomain || jQuery.support.cors) {
								...
								return {
									send : function(headers, complete) {
										...
										// Need an extra try/catch for cross
										// domain requests in Firefox 3
										try {
											for (i in headers) {
										...
										if (!s.async) {
											...
											// (IE6 & IE7) if it's in cache and
											// has been
											// retrieved directly we need to
											// fire the callback
											setTimeout(callback, 0);
										} else {
											handle = ++xhrId;
											if (xhrOnUnloadAbort) {
												// Create the active xhrs
												// callbacks list if needed
				...
				"*" : [ function(prop, value) {
					...
					if (parts) {
						...
						if (unit !== "px" && start) {
							...
							// point
							// Prefer the current property, because this process
							// will be trivial if it uses the same units
							// Fallback to end or a simple constant
							start = jQuery.css(tween.elem, prop, true) || end
									|| 1;
						...
						// animation
						tween.end = parts[1] ? start + (parts[1] + 1) * end
								: end;
					}
					return tween;
				} ]
			...
			function Animation(elem, properties, options) {
				var result, index = 0, tweenerIndex = 0, length = animationPrefilters.length, deferred = jQuery
					...
					var currentTime = fxNow || createFxNow(), remaining = Math
							.max(0, animation.startTime + animation.duration
									- currentTime),
					...
					deferred
							.notifyWith(elem, [ animation, percent, remaining ]);
					...
					createTween : function(prop, end, easing) {
						var tween = jQuery.Tween(elem, animation.opts, prop,
								end, animation.opts.specialEasing[prop]
			...
			function defaultPrefilter(elem, props, opts) {
				...
				if (length) {
					...
					for (index = 0; index < length; index++) {
						prop = handled[index];
						tween = anim.createTween(prop, hidden ? dataShow[prop]
								: 0);
			...
			function Tween(elem, options, prop, end, easing) {
				return new Tween.prototype.init(elem, options, prop, end,
						easing);
			}
			jQuery.Tween = Tween;
			...
			jQuery.fn
					.extend({
						fadeTo : function(speed, to, easing, callback) {
							...
							// 0
							return this.filter(isHidden).css("opacity", 0)
									.show()
			...
			function genFx(type, includeWidth) {
				...
				// if we include width, step value is 1 to do all cssExpand
				// values,
				// if we don't include width, step value is 2 to skip over Left
				// and Right
			...
			jQuery.fn.offset = function(options) {
				if (arguments.length) {
					return options === undefined ? this : this
							.each(function(i) {
			...
			jQuery.offset = {
				...
				setOffset : function(elem, options, i) {
					...
					// set position first, in-case top/left are set even on
					// static elem
					if (position === "static") {
					...
					var curElem = jQuery(elem), curOffset = curElem.offset(), curCSSTop = jQuery
							.css(elem, "top"), curCSSLeft = jQuery.css(elem,
					...
					// need to be able to calculate position if either top or
					// left is auto and position is either absolute or fixed
			...
			jQuery.fn
					.extend({
						...
						position : function() {
							...
							// incorrectly be 0
							offset.top -= parseFloat(jQuery.css(elem,
									"marginTop")) || 0;
							offset.left -= parseFloat(jQuery.css(elem,
									"marginLeft")) || 0;
			...
			// anonymous
			// AMD modules. A named AMD is safest and most robust way to
			// register.
